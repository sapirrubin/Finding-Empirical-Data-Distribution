...
#GA
def genetic_algorithm_no_bitstring(objective,samples, dis_to_fit, n_iter, n_pop, r_cross, r_mut):
    # initial population of random bitstring
#     pop = [randint(0, 2, len(samples)).tolist() for _ in range(n_pop)]
    
    pop_indexes = [randint(1, 2, len(samples)).tolist() for _ in range(n_pop)]
    pop = [decode(samples, p_index) for p_index in pop_indexes]
    
    # keep track of best solution
    best = pop[0]
#     best_eval,best_eval_index = objective(decode(samples, pop[0]))
    best_eval,best_eval_index = objective(pop[0],dis_to_fit)
    # enumerate generations
    for gen in range(n_iter):
        # evaluate all candidates in the population
        scores = [objective(p,dis_to_fit)[0] for p in pop]
        scores_index = [objective(p,dis_to_fit)[1] for p in pop]
        ...
        for p in pop:
            p.sort()
    return [best, best_eval,best_eval_index]
...
#     best, score, x_min_index = genetic_algorithm_no_bitstring(objective=KS_test, samples=samples, dis_to_fit=disribution ,
#                                                  n_iter=n_iter, n_pop=n_pop, r_cross=r_cross, r_mut=r_mut)
    print('Done!')
    print('f() = %f' % (score))
    result_x_min = best[0]
...
best = find_empirical_dist('pl', data, emprical_dist=degree_data)
best = find_empirical_dist('exp', data, emprical_dist=degree_data)
best = find_empirical_dist('normal', data, emprical_dist=degree_data)
#Existed
#power-law
...
best = find_empirical_dist('pl', data)
best = find_empirical_dist('exp', data)
best = find_empirical_dist('normal', data)
#Existed
#power-law
result_alpha = estimate_alpha(x=data,x_min=data[0])
n_cdf = len(data)
